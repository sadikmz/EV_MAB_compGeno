...
EV_MAB_reads_coverage_geneID_GO_terms_wide %>%
  ...
  filter(MB ==1) %>%
  filter(EV !=1) %>%
  filter(gene_id=="EVBD.1.027798") %>%
  head()


# EV_MAB_reads_coverage_geneID_GO_terms_wide.v2
...
# distinct(reads_genome)
  ...
  # filter(syn_cov < 0.25) %>%
  mutate(
    # set fraction_coverage to 0 for non-syntenic regions and cds regions whose interspecies reads mapping coverage is less than 0.25%
    syn_frac_cov = case_when(
      
      # This may be de novo evolved genes who are not syntenic and do not have reads support
      synteny == "non_syntenic" & fraction_overlap < 0.25 ~ "0",
...
# EV_bedadeti_vs_MAB_vendiag <-
  ...
  venn_data_process$regionData$item[[1]] %>% 
    ...
    distinct(gene_id) %>%
    filter (str_detect(gene_id,"047050"))
...
ggsave(paste0(DATA_DIR, "plot_out/EV_mazia_bedadeti_vendiag.allMapped.synteny.02.png"), width=10, height=4)

MA_vs_EV_vendiag+MB_vs_EV_vendiag
ggsave(paste0(DATA_DIR, "plot_out/MA_MB_vendiag.allMapped.synteny.02.png"), width=6, height=4)


(EV_mazia_vs_MAB_vendiag/MA_vs_EV_vendiag)
# /(MA_vs_EV_vendiag|MB_vs_EV_vendiag)

ggsave(paste0(DATA_DIR, "plot_out/EV_mazia_top_vs_MA_bottom_genes_vendiag.allMapped.synteny.02.pdf"), width=9, height=9)
