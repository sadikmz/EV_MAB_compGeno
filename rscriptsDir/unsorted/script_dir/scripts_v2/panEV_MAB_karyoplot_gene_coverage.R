...
MB_specific_genes_grange <-
  ...
  dplyr::rename(seqnames = seqid,
                coverage = frac_ovrerlap_syn) %>%
  dplyr::filter(str_detect(seqnames,"chr02")) %>%
...
MB_genome_bed <-
  ...
  filter(str_detect(seq.name,"chr")) %>%
  mutate(seq.name = str_remove(seq.name,"B")) %>%
  dplyr::rename(seqnames = seq.name) #%>%
  # as_granges()
# filter(chr == "chr01")
...
kpRect(kp, data = MB_repeats, y0=0, y1=1, col="#09ebd4", border=NA, r0=-.31, r1=-0.6)
kpRect(kp, data = MB_gene, y0=0, y1=1, col="#5809eb", border=NA, r0=-.61, r1=-0.9)
...
kp <- plotKaryotype(genome=MAB_genome_bed,  ideogram.plotter = NULL, plot.type=2, cex=1 )
# kpRect(kp, data = MA_gene, y0=0, y1=1, col="#5809eb", border=NA, r0=0, r1=0.2)
kpRect(kp, data = MB_gene, y0=0, y1=1, col="#5809eb", border=NA, r0=0, r1=0.3)

kpRect(kp, data = MB_repeats, y0=0, y1=1, col="#09ebd4", border=NA, r0=0, r1=-0.3)

# PanEV reads coverage against MA
kp <- kpBars(kp, panEV_MB_genomecov_grange, 
             y1=panEV_MB_genomecov_grange$coverage, 
             ymax = max(panEV_MB_genomecov_grange$coverage),
             col="#8c8ca1", r0=0.31, r1=1.2,  border=NA )
