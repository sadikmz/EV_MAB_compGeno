...
#
# Usage: sbatch NLR_loci_vs_NLR_genes.sh  |or|  bash NLR_loci_vs_NLR_genes.sh
...
GENOME_DIR="${GENOME_DIR:-/path/to/ref-genomes}"       # FASTA files: EV_mazia.fna, EV_bedadeti.fna, Musa_acuminata.fna, Musa_balbisiana.fna
QUERY_DIR="${QUERY_DIR:-../pred_prot/PF00931_out}"     # per-taxon NB-ARC FASTA files (from NLR_predicted_prot.sh)
NCBI_NT_DB="${NCBI_NT_DB:-${HOME}/NCBI_NT/BLASTDB/nt}" # NCBI nt BLAST db for tblastx validation
NLR_ANNOT_DIR="${NLR_ANNOT_DIR:-NLR-annotator.out}"    # NLR-Annotator output (from NLR-annotator.sh)

CPUS="${SLURM_CPUS_PER_TASK:-48}"    # tblastn threads
CPUS_BLASTX="${TBLASTX_CPUS:-16}"   # tblastx threads (fewer — NCBI nt is I/O-bound)

BLAST_OUTDIR="${BLAST_OUTDIR:-hmmNLR_mapping_out}"
BLASTX_OUTDIR="${BLASTX_OUTDIR:-unmapped_NLR_blastx}"

# Genome keys → FASTA paths; GENOME_ORDER gives deterministic iteration order.
# Override GENOME_DIR via environment to point to the correct directory.
