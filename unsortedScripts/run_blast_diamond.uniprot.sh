...
"$diamond" makedb -in "$uniprot_dir"/uniprot_sprot.fasta.gz --d "$uniprot_dir"/uniprot_sprot.fasta.gz

"$diamond" blastp \
--db "$uniprot_dir"/uniprot_sprot.fasta.gz \
--query $QPROT_DIR/EV_MAB.novel_genes.fa \
--ultra-sensitive \
--masking 0 \
--out EV_MAB.novel_genes.uniprot_db.blastp.e05.usensitive.tsv \
...
# extract blast hits
zcat EV_MAB.novel_genes.uniprot_db.blastp.e05.usensitive.tsv.gz | awk '{if ($5 > $6) print $1"\t"$6"\t"$5"\t"$12"\t"$11"\t"$10"\t"$11/$2"\t"$3"\t"$18; else print $1"\t"$5"\t"$6"\t"$12"\t"$11"\t"$10"\t"$11/$2"\t"$3"\t"$18}' |  awk '{if ($4> 25) print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9}'  OFS='\t' > EV_MAB.novel_genes.uniprot_db.blastp.e05.usensitive.query.bed
zcat EV_MAB.novel_genes.uniprot_db.blastp.e05.usensitive.tsv.gz | awk '{if ($7 > $8) print $3"\t"$8"\t"$7"\t"$12"\t"$11"\t"$10"\t"$11/$4"\t"$1"\t"$18; else print $3"\t"$7"\t"$8"\t"$12"\t"$11"\t"$10"\t"$11/$4"\t"$1"\t"$18}' |  awk '{if ($4> 25) print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9}'  OFS='\t' > EV_MAB.novel_genes.uniprot_db.blastp.e05.usensitive.ref.bed
