...
## EV_landrace SRR files in PRJNA344540, PRJNA252658, PRJNA170519 and PRJEB58004 were saved as EV_PRJNA344540.txt, EV_PRJNA252658.txt, EV_PRJNA170519.txt and EV_PRJEB58004.txt, respectively.
...
esearch -db sra -query PRJNA170519 | efetch -format runinfo | cut -d ',' -f 1 | grep SRR | grep -E 'SRR610420' | xargs -n 1 -P $cpus fastq-dump --split-files --gzip --skip-technical
esearch -db sra -query PRJEB58004  | efetch -format runinfo | cut -d ',' -f 1 | grep SRR | grep -E 'ERR10695598' | xargs -n 1 -P $cpus fastq-dump --split-files --gzip --skip-technical

## Trim adapter sequences and remove poor quality reads. Downloaded fastq files were saved as EV_sra_list.fofn
ls -lht *.fastq.gz | awk '{print $9' | sed 's/_?.fastq.gz//g' > EV_fastq_list.fofn

for i in $(cat EV_sra_list.fofn)
do 
...
## Trim adapter sequences and remove poor quality reads. Downloaded fastq files were saved as EV_sra_list.fofn
ls -lht *.fastq.gz | awk '{print $9' | sed 's/_?.fastq.gz//g' > musaSpp_fastq_list.fofn

# adapter and poor quality reads trimming 
for i in $(cat musaSpp_fastq_list.fofn)
do 
