...
class ReadMapping:
    ...
    def remove_duplicates_dry_run(self, memory_size, picard_jar):
        """Print commands for removing duplicates with Picard or samtools."""
        bam_file = f"{self.output_dir}/{self.out}.allMapped.sorted.bam"
        commands = []
        if picard_jar:
            picard_cmd = f"java -Xmx{memory_size} -jar {picard_jar} MarkDuplicates INPUT={bam_file} O={self.output_dir}/{self.out}.allMapped.sorted.markdup.bam M={self.output_dir}/{self.out}.allMapped.sorted.markdup.bam.metrics.txt REMOVE_DUPLICATES=True"
            commands.append({'command': picard_cmd, 'type': 'picard_markdup'})
        else:
            commands.append({'command': f"echo 'Skipping Picard MarkDuplicates: picard_jar not provided'", 'type': 'picard_markdup'})
...
def main():
    ...
    for _, row in sra_df.iterrows():
        ...
        for sra_id in sra_ids:
            ...
            commands.extend(read_mapping.remove_duplicates_dry_run(memory_size, args.picard_jar))
            ...
            coverage_analysis = CoverageAnalysis(bam_file, output_dir, bed_file, sra_id, args.genome_prefix, args.cpus, args.qualimap, args.bamcoverage)
