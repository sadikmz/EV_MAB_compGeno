...
class GenomicAnalyzer:
    ...
    def section_2_11_quality_control_dry_run(self):
        ...
        for _, row in self.sra_df.iterrows():
            ...
            commands.append({'genotype': genotype, 'bioproject': bioproject, 'command': download_cmd, 'type': 'download'})
...
def main():
    ...
    for _, row in commands_df.iterrows():
        print(f"Genotype: {row['genotype']}, SRA: {row.get('sra', 'N/A')}, Type: {row['type']}")
