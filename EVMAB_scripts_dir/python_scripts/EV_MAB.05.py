...
class GenomicAnalyzer:
    ...
    def download_trim_ngs(self):
        """Execute SRA download and trimming for rows 2 and 3."""
        status_data = []
...
def main():
    parser = argparse.ArgumentParser(description="Execute: Download and trim SRA reads")
    parser.add_argument("--sra_metadata", required=True, help="Path to SRA CSV file (e.g., sra.csv) and containing the following columns: genotypes, species, cultivated_wild,	size, sra, bioproject")
    parser.add_argument("--output_dir", default="results", help="Output directory")
    parser.add_argument("--cpus", default=8, type=int, help="Number of CPUs for parallel processing")
    args = parser.parse_args()

    analyzer = GenomicAnalyzer(args.sra_csv, args.output_dir, args.cpus)

    # Run execution for download and trim
    status_df = analyzer.download_trim_ngs()
    print("SRA Download and Trim Status")
    print(status_df)
