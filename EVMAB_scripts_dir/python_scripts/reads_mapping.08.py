...
class CoverageAnalysis:
    def __init__(self, bam_file, output_dir, gene_bed, reads, genome_prefix, cpus=28, bedtools_path=None, qualimap_path=None, bamcoverage_path=None):
    ...
    def generate_gene_coverage(self, memory_size, genome_file):
        """Execute command to generate coverage for all mapped reads in genic regions."""
        cov_file = os.path.join(self.output_dir, f"{self.reads}.allMapped.reads_gene.cov.bed")
        try:
            cmd = f"{self.bedtools_path} bamtobed -i {self.bam_file} | {self.bedtools_path} coverage -a {genome_file} -iobuf {memory_size} -b - > {cov_file}"
            subprocess.run(cmd, shell=True, check=True)
            return {'status': f'Generated gene coverage: {cov_file}'}
        except subprocess.CalledProcessError as e:
            return {'status': f'Failed gene coverage: {e}'}

def main():
    parser = argparse.ArgumentParser(description="Execute genomic analysis (Sections 2.12-2.15)")
    parser.add_argument("--query_genome", required=True, help="Path to reference genome FASTA")
    ...
    parser.add_argument("--memory_size", required=True, help="Memory size for Picard, Qualimap, and bedtools (e.g., 50G)")
    ...
    for _, row in sra_df.iterrows():
        ...
        for sra_id in sra_ids:
            ...
            read_mapping = ReadMapping(args.query_genome, fread, rread, output_dir, args.genome_prefix, args.cpus, args.bwa_mem2, args.samtools)
            ...
            coverage_analysis = CoverageAnalysis(bam_file, output_dir, gene_bed, sra_id, args.genome_prefix, args.cpus, args.bedtools, args.qualimap, args.bamcoverage)
            status.append({'step': f'qualimap_{sra_id}', 'status': coverage_analysis.run_qualimap(memory_size)})
            status.append({'step': f'bigwig_coverage_{sra_id}', 'status': coverage_analysis.generate_bigWigCoverage()})
            status.append({'step': f'gene_coverage_{sra_id}', 'status': coverage_analysis.generate_gene_coverage(memory_size, args.genome_file)})
