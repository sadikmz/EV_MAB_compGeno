...
class ToolChecker:
    ...
    def check_tools_dry_run(self):
        """Check if tools are installed and available in PATH, generate stdout report."""
        commands = []
        not_installed = []
        for tool in self.tools:
            tool_path = shutil.which(tool, path=self.tools_path)
            if tool_path:
                self.report.append({'tool': tool, 'status': 'Installed', 'path': tool_path})
            else:
                self.report.append({'tool': tool, 'status': 'Not Installed', 'path': 'N/A'})
                not_installed.append(tool)
            # For picard, derive jar path if installed
            if tool == 'picard' and tool_path:
                picard_dir = os.path.dirname(tool_path)
                picard_jar = os.path.join(picard_dir, '..', 'share', 'picard', 'picard.jar')  # Adjust path as needed
                if os.path.exists(picard_jar):
                    self.report[-1]['path'] = picard_jar
                else:
                    self.report[-1]['status'] = 'Not Installed (jar not found)'
                    not_installed.append(tool)

        # Generate stdout report for not installed tools
        if not_installed:
            print("Tools Not Installed Report:")
...
class GFFtoBED:
    ...
    def convert_dry_run(self):
        """Print command to convert GFF to BED for genic regions using pybedtools."""
        # Simulate pybedtools usage
        cmd = f"pybedtools.BedTool('{self.gff_file}').filter(lambda x: x[2] == 'gene').saveas('{self.output_bed}')"
...
class ReadMapping:
    def __init__(self, ref_genome, fread, rread, output_dir, genome_prefix, cpus=28):
        ...
        self.genome_prefix = genome_prefix.upper()  # Capitalize prefix
        self.cpus = cpus
    ...
    def remove_duplicates_dry_run(self, memory_size):
        ...
        commands = []
        picard_cmd = f"java -Xmx{memory_size}G -jar $PICARD MarkDuplicates INPUT={bam_file} O={self.output_dir}/{self.out}.allMapped.sorted.markdup.bam M={self.output_dir}/{self.out}.allMapped.sorted.markdup.bam.metrics.txt REMOVE_DUPLICATES=True"
...
class CoverageAnalysis:
    ...
    def run_qualimap_dry_run(self, memory_size):
        ...
        commands = [
            {
                'command': f"qualimap bamqc -bam {self.bam_file} -outdir {qualimap_dir} -outfile {self.reads}_{self.genome_prefix}.qualimap -sd -c -nt {self.cpus} -outformat PDF:HTML -ip --java-mem-size={memory_size}G",
...
def main():
    ...
    parser.add_argument("--genome_prefix", required=True, help="Reference genome prefix (max 4 letters)")
    parser.add_argument("--memory_size", required=True, help="Memory size for Java tools (e.g., 100)")
    ...
    # Validate genome_prefix length and capitalize
    ...
    if 'output_dir' not in args:
        args.output_dir = os.path.join(args.workdir, f"pan{args.genome_prefix}_out")
    ...
    checker.check_tools_dry_run()
    # Print report to stdout
    report_df = pd.DataFrame(checker.report)
    print("Tools Report:")
    ...
    for _, row in sra_df.iterrows():
        ...
        for sra_id in sra_ids:
            ...
            read_mapping = ReadMapping(args.genome, fread, rread, args.output_dir, args.genome_prefix, args.cpus)
            ...
            # 4. Coverage analysis
            bam_file = os.path.join(args.output_dir, f"{args.genome_prefix}_{sra_id}.allMapped.sorted.markdup.bam")
            coverage_analysis = CoverageAnalysis(bam_file, args.output_dir, bed_file, sra_id, args.genome_prefix, args.cpus)
    ...
    commands_df.to_csv(os.path.join(args.output_dir, 'tool_and_analysis_commands_dry_run.csv'), index=False)
