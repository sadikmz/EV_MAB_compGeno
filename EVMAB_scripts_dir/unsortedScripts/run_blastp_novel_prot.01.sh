...
PROT_DIR="/home/data/salixomics/sadik/evmab_dir/ev_mab_prots"
...
makeblastdb -in $PROT_DIR/EV_MAB.prot.fa -input_type fasta -dbtype prot
...
awk '{if ($10 > $9) print $1"\t"$10"\t"$9"\t"$3"\t"$4"\t"$12; else print $1"\t"$9"\t"$10"\t"$3"\t"$4"\t"$12}' EV_MAB_novel_prot_self_blastp.e05.out | awk '{if ($3> 25) print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6 }'> EV_MAB_novel_prot_self_blastp.ref.bed 
