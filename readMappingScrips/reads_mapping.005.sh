...
for READS in SRR9734079 SRR9850640
	#$(cat MB_trim_out/MB_reads.list) 
	#SRR9850640 SRR9734079 ERR10695529 SRR6996489 
...
do 
	...
	#rename.sh in=$Fread in2=$Rread out="$Fread".rep out2="$Rread".rep prefix=$READS

	## map PE reads  
	# $APPSDIR/bwa-mem2/bwa-mem2 index $ref
	$APPSDIR/bwa-mem2-2.2.1_x64-linux/bwa-mem2 mem -t $cpus $ref "$Fread".rep "$Rread".rep -P | samtools view - -Sb -@$cpus | samtools view -b -@$cpus -F 4 | samtools sort - -@$cpus -o ${output_dir}/$out.allMapped.sorted.bam

	# REMOVE_DUPLICATES 
        samtools collate -o ${output_dir}/$out.allMapped.sorted.collate.bam ${output_dir}/$out.allMapped.sorted.bam -@$cpus
         ${output_dir}/$out.allMapped.sorted.bam
