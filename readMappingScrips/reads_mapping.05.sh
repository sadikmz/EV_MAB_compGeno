...
do 
	...
	# download reads
	esearch -db sra -query PRJNA432894 | efetch -format runinfo | cut -d ',' -f 1 | grep -E $READS | xargs -n 1 -P $cpus fastq-dump --split-files --gzip --skip-technical
	...
	#       M=${output_dir}/$out.allMapped.sort
